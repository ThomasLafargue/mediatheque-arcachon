...
def main():
    ...
    n, mini, maxi = cur.execute(
        "SELECT COUNT(*), MIN(date), MAX(date) FROM frequentation_ouverture").fetchone()
    ...
    for motif, nb in cur.execute("""
        ...
        FROM frequentation_ouverture GROUP BY motif ORDER BY 2 DESC LIMIT 10""").fetchall():
        print(f"  {nb:>6}  {motif}")

    print("\nJours par année :")
    for annee, nb, total in cur.execute("""
        SELECT SUBSTR(date,1,4), COUNT(*), SUM(nb_entrees)
        FROM frequentation_ouverture GROUP BY 1 ORDER BY 1""").fetchall():
        print(f"  {annee} : {nb:>4} jours, {total or 0:>8} entrées")

    print("\nValeurs suspectes :")
    for lib, sql in [
        ("nb_entrees NULL ou <= 0",
         "SELECT COUNT(*) FROM frequentation_ouverture WHERE nb_entrees IS NULL OR nb_entrees <= 0"),
        ("doublons de date",
         "SELECT COUNT(*) FROM (SELECT date FROM frequentation_ouverture GROUP BY date HAVING COUNT(*) > 1)"),
        ("jours > 3000 entrées (aberrants ?)",
         "SELECT COUNT(*) FROM frequentation_ouverture WHERE nb_entrees > 3000"),
    ]:
        print(f"  {cur.execute(sql).fetchone()[0]:>6}  {lib}")

    print("\nTop 5 des journées les plus fortes :")
    for d, e in cur.execute(
            "SELECT date, nb_entrees FROM frequentation_ouverture "
    ...
    nh, minh, maxh = cur.execute(
        ...
        "FROM frequentation_horaire_ouverture").fetchone()
    print(f"horaire : {nh} jours, du {minh} au {maxh}")
    seul_j = cur.execute("""
        SELECT COUNT(*) FROM frequentation_ouverture f WHERE NOT EXISTS
        (SELECT 1 FROM frequentation_horaire_ouverture h WHERE h.date = f.date)""").fetchone()[0]
    seul_h = cur.execute("""
        SELECT COUNT(DISTINCT date) FROM frequentation_horaire_ouverture h WHERE NOT EXISTS
        (SELECT 1 FROM frequentation_ouverture f WHERE f.date = h.date)""").fetchone()[0]
    print(f"jours au journalier sans détail horaire : {seul_j}")
    print(f"jours au détail horaire absents du journalier : {seul_h}")
    ecarts = cur.execute("""
        SELECT COUNT(*) FROM (
            SELECT h.date FROM frequentation_horaire_ouverture h
            JOIN frequentation_ouverture f ON f.date = h.date
    ...
    if ecarts:
        ...
        for d, sh, tj in cur.execute("""
            ...
            FROM frequentation_horaire_ouverture h JOIN frequentation_ouverture f ON f.date = h.date
    ...
    for h, nb, tot in cur.execute("""
        SELECT heure, COUNT(*), SUM(nb_entrees) FROM frequentation_horaire_ouverture
