...
OUTIL_SQL = {
    ...
    "description": (
        ...
        "FRÉQUENTATION -- DEUX SÉRIES PARALLÈLES (renommées le 06/08/2026, "
        "les tables `frequentation` et `frequentation_horaire` N'EXISTENT "
        "PLUS -- toute requête dessus échouera) : "
        "frequentation_ouverture / frequentation_horaire_ouverture = heures "
        "d'OUVERTURE au public uniquement (juillet-août lundi-samedi 10h-19h, "
        "reste de l'année mardi-samedi 10h-18h ; un jour absent = fermé au "
        "public, pas une donnée manquante) ; "
        "frequentation_brute / frequentation_horaire_brute = JOURNÉE ENTIÈRE "
        "sans filtre (tous jours et toutes heures, y compris passages de "
        "l'équipe -- écart de 8 à 15 %). C'est la série brute que l'équipe "
        "suit et compare depuis des années. Le nom de la table interrogée "
        "DOIT correspondre à la série annoncée dans le titre du tableau "
        "(le 06/08/2026, un tableau étiqueté « brut » avait été rempli avec "
        "les chiffres ouverture : erreur grave, à ne jamais reproduire). "
        "RÈGLE DE DIALOGUE : si la personne demande un point/bilan/comparaison "
        "de fréquentation SANS préciser, DEMANDE d'abord : « Veux-tu les "
        "chiffres en tenant compte des horaires d'ouverture, ou les données "
        "brutes journée entière (comparables à l'historique) ? » -- puis "
        "utilise la bonne paire de tables et affiche OBLIGATOIREMENT la série "
        "utilisée dans le TITRE de chaque tableau ou chiffre de fréquentation "
        "-- « (brut, journée entière) » ou « (heures d'ouverture) » -- sans "
        "exception, sinon les chiffres sont incomparables entre eux (confusion "
        "constatée le 06/08/2026). Structure des 4 tables : (date, nb_entrees) "
        "pour les journalières, (date, heure, nb_entrees) pour les horaires "
        "(ex. '14:00' = de 14h à 15h). "
        "COMPARAISONS ENTRE ANNÉES — RÈGLE ABSOLUE : une même date ne tombe "
        "PAS le même jour de semaine d'une année à l'autre (le 27/07 est un "
        "dimanche en 2025, un lundi en 2026). JAMAIS de tableau avec une "
        "seule colonne « jour » pour deux années : c'est illisible et faux. "
        "Deux présentations correctes au choix : (1) aligner par JOUR DE "
        "SEMAINE — lundi en face de lundi — en décalant les dates (utiliser "
        "STRFTIME('%w', date), 0=dimanche, et afficher la date propre à "
        "chaque année sur la ligne) ; (2) comparer les agrégats (total ou "
        "moyenne par jour ouvert de la période). Toujours calculer le jour "
        "de semaine par STRFTIME, jamais le déduire d'une autre année. "
        "TOTAUX ET MOYENNES : tout agrégat affiché (total, moyenne, écart en "
        "%) doit sortir d'une requête SUM()/AVG() dédiée — ne JAMAIS "
        "additionner de tête les lignes d'un tableau (erreur constatée le "
        "06/08/2026 : total annoncé 7 433 pour des lignes sommant à 8 433) ; "
